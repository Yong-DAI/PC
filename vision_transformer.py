...
from pgnbase_G import PGNbase_G
from pgnbase_G_1 import PGNbase_G_1
...
#         self.pgn_module_G = PGNbase_G(    **self.pgn_settings    )
#         self.pgn_module_G_1 = PGNbase_G_1(    **self.pgn_settings    )
        
#         self.pgn_module_Gfc = nn.Sequential(linear_layer(768, 25*2 * 256))
#         self.pgn_module_Gfc_1 = nn.Sequential(linear_layer(768, 25*2 * 256))
        
#         self.pgn_module_Gfc = nn.Sequential(
...
#         self.pgn_meta_Gfc_1 = nn.Sequential(
#             linear_layer(768, 128),
#             nn.ReLU(),
#             linear_layer(128,768)) 
    ...
    def forward_features(self, x, task_id=-1,task_key_norm= None, cls_features=None, train=False):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if self.use_g_prompt or self.use_e_prompt:
                ...
                res = self.e_prompt(x,task_id=task_id, task_key_norm= task_key_norm, prompt_mask=prompt_mask, cls_features=cls_features,trainable=train)
                e_prompt = res['batched_prompt']
                
#                 pgn_prompts_G_  = self.pgn_module_G(cls_features, self.maben)
                ########## (num_g_prompt, 2, g_prompt_length, num_heads, embed_dim // num_heads)
#                 G_prompt = rearrange(pgn_prompts_G, 'b (l n e) (h d) -> b l n e h d',l=2,n=2, h = self.num_heads).permute(1, 0, 2,3,4,5)
#                 pgn_prompts_G=rearrange(pgn_prompts_G_.unsqueeze(0), 'l b (n e) (h d) -> l b n e h d', n = 2, h = self.num_heads)
                
#                 pgn_prompts_G_1_ = self.pgn_module_Gfc_1(pgn_prompts_G_)
#                 pgn_prompts_G_1 = self.pgn_meta_Gfc_1(cls_features)
#                 pgn_prompts_G_1=repeat(pgn_prompts_G_1, 'b h -> b n h', n = 50)
#                 pgn_prompts_G_1_ = pgn_prompts_G_+pgn_prompts_G_1
            
#                 pgn_prompts_G_1_  = self.pgn_module_G_1(cls_features, self.maben)
#                 pgn_prompts_G_1= rearrange(pgn_prompts_G_1_.unsqueeze(0), 'l b (n e) (h d) -> l b n e h d', n = 2, h = self.num_heads)
                
#                 G_prompt  = torch.cat([pgn_prompts_G, pgn_prompts_G_1], dim=0)
...
#                             idx = torch.tensor([g_prompt_counter] ).to(x.device)
#                             g_prompt = G_prompt[idx].squeeze()
...
#             x = x[:, 1:(1 + self.total_prompt_len)] if self.class_token else x[:, 0:self.total_prompt_len]
    ...
    def forward(self, x, task_id=-1, task_key_norm= None,cls_features=None, train=False):
        res = self.forward_features(x, task_id=task_id, task_key_norm= task_key_norm, cls_features=cls_features, train=train)
