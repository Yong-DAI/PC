...
def train_and_evaluate(model: torch.nn.Module, model_without_ddp: torch.nn.Module, original_model: torch.nn.Module, 
    ...
    chech_path = datetime.datetime.now().strftime('log_%Y_%m_%d_%H_%M')
    ...
    for task_id in range(args.num_tasks):
        ...
        if args.output_dir and utils.is_main_process():
            Path(os.path.join(args.output_dir, 'checkpoint_%s'%chech_path)).mkdir(parents=True, exist_ok=True)
            
            checkpoint_path = os.path.join(args.output_dir, 'checkpoint_{}/task{}_checkpoint.pth'.format(chech_path, task_id+1))
            state_dict = {
